...
下次看到 `GPU-Util=100%` 但 token/s 上不去，先别急着扩容。建议收藏上面这张三步清单：**先判定是在等数据、等计算，还是等调度，再决定钱该花在带宽、算力还是 serving 系统上。**

如果你正在做 LLM serving，可以把这篇转发给负责推理优化的同事。下一篇我会继续拆 KV Cache：上下文变长后，显存为什么会从“装权重”变成“养会话”。
