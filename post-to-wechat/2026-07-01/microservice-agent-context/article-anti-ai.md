...
summary: "别把十几个微服务直接丢给 Agent，先补服务地图、接口契约和本地验证证据。"
tags:
  - type/article
...
你把一个跨订单、库存、支付的 user story 丢给 Agent。它改得很快，PR 也像模像样，但只看了订单服务，没发现库存事件的字段约定已经变了。联调一跑，链路炸在最不起眼的接口边界上。

这是跨微服务用 Agent 最容易踩的坑：不是模型不会写代码，而是它没有服务地图，也没有一条能在本地跑起来的契约验证线。

把所有仓库放进一个 workspace 有用，但只能解决“看得到代码”。要让 Agent 真能改跨服务需求，还要补三件事：

1. 全局视图：先知道系统由哪些服务组成。
2. 精准上下文：按需读取服务边界、领域概念和接口协议。
3. 验证闭环：改完代码后，用契约测试和 mock server 自查。

![跨微服务 Agent 三层工作台](imgs/microservice-agent-workbench.png)

有了服务地图、协议边界和验证闭环，Agent 才不是在一堆仓库里盲翻文件，而是在一张工程地图上做局部探索和局部验证。
...
但 workspace 只解决“看得到”的问题，不会自动告诉 Agent “先看什么”。
...
Anthropic 在 context engineering 里强调过同一个取舍：上下文是有限资源。Agent 需要的是高信号、按需加载的材料，不是“所有资料都来一点”。

所以根目录要放的不是一份巨型说明书，而是一张服务地图。

![workspace 只是入口，不是上下文](imgs/workspace-is-not-context.png)

## 根 AGENTS.md 做路由，服务 AGENTS.md 做边界

我会把跨微服务 Agent 的文档拆成两层。
...
这和新人入组很像。你不会一上来塞给他 300 页系统全景文档，而是先给系统地图，再告诉他：这个需求涉及订单、库存和支付，先看这三个服务的边界和契约。

好的上下文结构不负责替 Agent 写答案，它负责告诉 Agent 下一步该去哪里找答案。

![根 AGENTS.md 做路由，服务 AGENTS.md 写边界](imgs/agents-md-routing-map.png)
...
能从代码或规格生成的内容，尽量不要只靠手写。

OpenAPI、GraphQL schema、protobuf、AsyncAPI、数据库 migration、事件样例、Pact 契约文件，对 Agent 更有价值，因为它们更接近系统真实边界。

OpenAPI 官方定位很直接：它让人和计算机在不看源代码、不抓包的情况下理解 HTTP API 的能力，并且可以被文档、代码生成和测试工具使用。

对 Agent 来说，这决定了它能不能在本地复现另一个服务，而不是凭 README 猜接口。
...
![手写文档解释意图，机器协议锁住边界](imgs/protocol-as-hard-context.png)
...
![契约测试让 Agent 自己跑出证据](imgs/contract-test-loop.png)

这条链路比“让 Agent 把整个系统跑起来”现实得多，也更容易在普通开发机上复现。

Agent 不需要每次都等完整环境。只要能证明当前服务没有破坏约定，局部改动就可以先进入可评审状态。
...
如果团队已经有十几个微服务，我不会第一天就要求大家补齐完美文档。

我会按风险顺序做三步：

第一步，选一条高频 user story，把它涉及的 3 到 5 个服务放进 virtual monorepo。根目录只写服务地图和启动方式，不写长篇设计史。

第二步，只给这几个服务补局部 `AGENTS.md`：职责边界、领域词、关键 API、测试命令。旧文档先不搬，避免把过期信息带进上下文。

第三步，把最关键的服务间接口做成 OpenAPI spec 或 Pact 契约。先让 Agent 跑通一条 contract test，再逐步扩大覆盖面。

这比“先建设完整知识库，再让 Agent 上岗”更现实。

Agent 工程化不追求资料库看起来完整，先追求每次改动都有足够上下文和可验证证据。
...
如果团队只准备做一件事，不要先把所有 README 整理漂亮，先把“服务地图 + 机器可读协议 + 契约测试”连起来。

这套闭环会逼着文档保持克制，也会逼着 Agent 拿局部证据说话。

跨微服务用 Agent，不怕它看不到全部代码，怕的是它看到了太多，却没有一条可靠的验证线。

收藏这张三层清单。下次准备把跨服务 user story 丢给 Agent 前，先问三个问题：它知道该读哪几个服务吗？它知道不能破坏哪份契约吗？它改完以后能自己跑出证据吗？

如果这三个问题都答不上来，就别急着让 Agent 写代码，先给它搭工作台。后面我会继续拆一份跨微服务 Agent 的目录模板，想要模板可以留言「微服务」。
