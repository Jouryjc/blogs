...
普通 RAG 很可能给你一段“Token Cache 由 Redis 提供后端”的文档。检索没错，答案却没出来。要找的其实是另一条跨文档的依赖链：
...
**单跳事实查询继续用 RAG；跨文档依赖、影响分析和因果链，再考虑 Context Graph。**

![](imgs/context-graph-comparison.png)
...
我会先做混合检索，不会一上来迁移整套 RAG。Microsoft GraphRAG 的 Local Search 也是把知识图谱中的实体关系与原始 text chunks 一起放进检索上下文。图负责接关系，文本负责交付证据。

## 第一层：Extract，把文本变成能连接的三元组

**第 1 步：用失败查询定义关系。**

先写出要支持的问题，再反推最小关系集合。故障影响分析的第一版可能只需要 `depends_on`、`backed_by`、`runs_on` 和 `owned_by`。关系越多，抽取和评估越难；先选一个高价值关系，更容易验证图有没有用。

**第 2 步：抽取实体和关系，不急着做 embedding。**
...
一旦别名没有合并，边都在，路径还是断的。最小实现至少需要大小写与空白归一化、标点清洗、显式 alias 表，以及无法自动确认时的人工复核队列。

## 第二层：Connect，让每条边都带“收据”

**第 4 步：同时构建正向边和反向边。**

“A 依赖什么”要沿正向边查，“什么依赖 A”要沿反向边查。分别维护两张邻接表即可：
...
存两份方向会增加一点内存，但能避免影响分析时全图扫描。对依赖图来说，这笔交换通常值得。

**第 5 步：多个来源支持同一条边，就提高权重。**

两份独立文档都说“认证服务依赖 Token Cache”，这条边比单一来源更可信。最小做法是合并相同的 `subject + relation + object`，记录来源集合；后续再叠加文档可信度、更新时间和抽取置信度。

但十份互相复制的文档不是十个独立证据。来源去重比单纯计数更重要。

**第 6 步：provenance 必须挂在边上。**
...
候选答案不再是 Top-K 文档，而是一组路径。排序时至少要看路径完整性、边的来源与可信度、跳数、关系类型，以及证据是否过期或冲突。

**第 9 步：只把相关子图交给模型。**

最终上下文只保留路径上的节点、连接节点的边，以及每条边对应的原始片段。
...
验收标准也要提前写清楚：普通 RAG 的单跳准确率不能明显退化；多跳问题要能返回完整路径；每一跳都能打开原始证据；图中没有路径时，系统必须敢于说不知道。

![](imgs/context-graph-poc-checklist.png)
...
相反，如果高价值问题集中在服务依赖、组织责任、合规追踪、事件影响、跨文档调查，图会把“模型猜关系”变成“系统查路径”。
...
## 先拿 20 个失败问题做一次“点还是链”标注

如果你正在做企业知识库或 Agent 记忆，先把文中的 9 步清单收藏下来，再拿最近 20 个失败问题做一次“点还是链”的标注。这个结果会比任何框架选型更早告诉你，团队到底需不需要一张图。
