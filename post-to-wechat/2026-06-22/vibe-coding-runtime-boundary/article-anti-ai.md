...
summary: "AI 写出来的内部工具最危险的地方，往往不是按钮不好用，而是按钮背后没有身份、权限、审批和审计。文末附上线前清单。"
tags:
  - type/article
  - topic/agent-runtime
  - topic/agent-safety
...
| 用户身份怎么传递 | 每次敏感操作都能落到具体用户，而不是共享账号 |
...
下次 Agent 又很快帮你做完一个后台按钮，先别急着接生产库。把这张表当成上线前清单：先看身份、权限、审批、审计有没有落到运行时。

觉得有用可以收藏。关注「蒸馏小余」，下一篇继续拆 Agent 生成代码进入生产环境前，哪些 guardrails 该写在系统里，而不是写在 prompt 里。
