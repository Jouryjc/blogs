...
问题不一定是模型不够强，而是任务边界给错了：你让模型重新生成最终页面，却期待它只改一个局部。

论文 NL2Dashboard 给出的办法很克制：模型不直接维护整份 HTML，只修改一份结构化中间表示（IR）；页面交给确定性渲染器组装。论文实验中，它完成了全部修改任务，生成开销比值也从端到端方案常见的 1～3，降到生成时的 0.58、局部修改时的 0.02～0.43。
...
论文把它叫做 Reason-then-Render：模型处理开放判断，代码处理可重复装配。
...
如果 `add` 需要新的数据分析，Coder 再生成新的制品。随后 `IRModify` 依次更新配置，`DBCompile` 重新组装页面。

模型不能再靠一段看似合理的 HTML 假装完成任务。每次修改都落到有限动作、明确目标和可检查结果上。

对 Agent 工程来说，这比“再写一版更强的 Prompt”更有用。自然语言入口可以自由，执行层必须尽量收敛。
...
我的判断是，多 Agent 系统最容易做过头的地方，就是把“多检查几遍”当成可靠性。该先设计的是交接物：
...
这些结果支持“IR 能减少重复生成、提高修改可控性”，但有三条边界要记住：
...
它把模型从最终产物前面往后退了一步。模型负责判断，IR 负责交接，普通代码负责兑现。Agent 一旦开始频繁修改真实系统，这一步后退，往往比换一个更强模型更重要。
