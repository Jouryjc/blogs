...
summary: "AgentJudgeBench 给 LLM Judge 出了一套工具调用考卷：难题、无参考答案时，换大模型、加 CoT、调温度都没突破共同盲区。"
tags:
  - type/article
  - topic/agent-design
  - topic/agent-runtime
...
你的 Agent 跑完一套工具调用测试，LLM Judge 给出 92 分。第二天，团队换了一个更大的 Judge，分数变成 95。看起来评测升级了，但有个问题一直没人回答：**这位 AI 裁判自己判得准吗？**

ServiceNow AI 的新论文 [AgentJudgeBench](https://arxiv.org/html/2608.26623v1) 把裁判请上了考场。结果并不轻松：任务变难、又没有参考调用轨迹时，六个不同规模的 Judge 对同一个 Generator 的区分力开始收缩；换更大的模型、加思考链、调温度，都没有消掉这类共同盲区。

先说明口径。论文测的是 LLM Judge 与一套程序评分器的 **alignment**，不是 Agent 在现实世界里的任务成功率。即便如此，结论已经足够影响工程决策：**LLM Judge 可以补语义，但不该单独充当 Agent 评测的真相层。**
...
判分拆成四项：工具有没有选对、参数 key 是否完整、调用顺序是否匹配、用户要求是否覆盖。程序评分器也计算同样四项，论文再比较 LLM Judge 的分数离程序分数有多远。

实验最终保留 53,608 条可解析的 Generator 输出。每条输出交给六个 Judge，再成对比较 with-GT 与 without-GT，得到 321,648 个 paired evaluation 单元；如果把两个条件分别计数，就是 643,296 个 verdict。这里不能把“32 万组成对实验”缩写成“32 万次单独调用”，否则条件维度会被吃掉。
...
原因并不神秘。没有参考轨迹时，Judge 要同时从自然语言问题、工具 Schema 和生成结果中重建依赖关系。遇到 fan-in、diamond 或 loop-like 结构，它不仅要看单个调用对不对，还要追踪跨分支的先后约束。题目越含糊，裁判自己要完成的推理越接近一次新的规划任务。

**工作流结构本身也是难度。** 在论文的 with-GT 拓扑分析里，fan-out 约为 93%，fan-in 约为 83%，最易与最难相差约 10 个百分点。并行分支如果彼此独立，可以拆开检查；多个分支汇入同一个后续调用时，Judge 必须同时追踪前置结果。这个结论只对该合成基准负责，但它提醒我们：Agent 评测不能只按“问题类型”切片，还要按依赖拓扑切片。
...
它们不是在不同地方偶尔看走眼，而是在同一类结构问题上一起失灵。

## 六个裁判都同意，也可能只是一起乐观

还有一个更容易误导评测平台的结果。

有 GT 时，Judge 之间的平均一致率是 79.1%，Cohen's κ 约为 0.419；没有 GT 时，Judge 间一致率反而升到 92.6%，κ 约为 0.559。
...
按直觉，给 Judge 参考轨迹应该更容易判准。实验里，QwQ-32B 和 GPT-OSS-120B 确实从 GT 中获益，但两个前沿 Judge 反而下降：

- 论文内部命名的 GPT-5.4：-1.5 个百分点；
- Gemini-2.5-Pro：-3.9 个百分点。

作者把这一现象解释为 over-anchoring：Judge 看到参考轨迹后，过度依赖它的表面结构，不再独立检查生成结果是否可能功能等价。

论文又做了一组 corrupted-GT 控制实验，故意把其他样本的错误参考轨迹交给 Judge。Gemini-2.5-Pro 的表现仍和看到正确 GT 接近，差距只有 0–1.8 个百分点；QwQ-32B 发现参考不可信后，表现几乎回到 without-GT，差距不超过 0.2 个百分点。
...
差异最大的指标是参数结构。程序评分器会惩罚参考轨迹里没有出现的额外参数 key；人类标注者却可能认为，只要它符合 Schema、也不改变调用语义，就不该扣分。
...
评测条件也会改写排名。QwQ-32B 在 15 个 with-GT 配置中拿了 10 个第一，却没有在任何 without-GT 配置中排名第一；没有参考轨迹时，前沿 Judge 只在共同天花板内小幅领先。选 Judge 前至少要说清三件事：生产里有没有可靠 GT，要校准结构规则还是人工语义，发布门禁面对的是 easy 回归题还是 hard 长尾题。拿一张总榜替代这三项选择，后续每次换任务分布都要付出误判成本。

## 先别急着加 CoT，评分结构更值得试

碰到 Judge 不稳定，常见反应是让模型多想一会儿，或者把温度调低。AgentJudgeBench 的消融给这两种办法泼了冷水。

QwQ-32B 开关 thinking 后，平均 GT 差异只有 +0.11 个百分点，单个实验格最大不超过 0.3。温度实验的第一组配对最大 spread 不超过 0.6 个百分点，第二组不超过 0.25。

这不等于“所有 Judge 的 CoT 和温度都没用”。实验只覆盖有限模型配对。它说明的是：在这套结构匹配任务中，先调解码参数，收益很可能小于重写评分规则。

论文测到的最大配置杠杆，是逐指标、结构化的 JSON rubric。

在 Qwen3-32B Judge × Llama-3.3-70B Generator 上，它比自由文本 Prompt 高 4.8–6.5 个百分点。但换到 QwQ-32B × SmolLM3-3B，easy 只提升 3.9、medium 提升 2.4，hard 还反转成 -0.8。
...
此外，easy → medium 的难度改写只有 58.1% 被三个 meta-judge 一致认定为确实变难；medium → hard 才达到 93.9%。所以主结论更应该看 medium 到 hard 的退化，不要把三档难度理解成同样精准的标尺。

论文还把一个不可复现的 Azure GPT-5 preview 快照内部称作 GPT-5.4，并让它同时参与生成、判卷和难度改写。正文里的相关数字只能当作该快照在该实验设置中的结果，不能被读成一个公开产品型号的稳定能力。论文也没有把这些 Judge 接进 RLHF、DPO 或奖励训练，因此“难题上的分数压缩会污染训练信号”目前只是值得验证的风险，不是已经跑完的因果结论。

论文指出的是一个评测盲区，不是宣布 LLM Judge 失去价值。

LLM Judge 仍然适合处理语义、解释和开放判断；问题在于，我们不能拿一个同样会受难度、Prompt 和参考答案影响的模型，替代全部确定性证据，再把它的单一分数叫作“客观评测”。

如果你的团队正在搭 Agent 评测，把上面的四层防线放进方案评审：先写清哪些事实由程序验证，哪些语义交给 LLM，哪些分歧必须重放，哪些困难样本要由人看。

收藏的不是一个 Judge 排名，而是一条以后每次换模型、换 Prompt、换任务都能重新校准的评测路线。

如果你想看我把四层防线落成一套可运行的 eval schema，留言 **Judge**。下一篇可以继续拆字段、失败类型、分歧队列和发布门禁规则。
