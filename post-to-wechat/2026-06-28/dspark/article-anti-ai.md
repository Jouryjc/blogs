...
你做过 Agent 或代码生成服务，就会遇到一个很现实的等待：用户不只等第一个字，还在等解释、补丁、工具结果和下一步计划一段段吐完。

DeepSeek 这篇 DSpark，讲的不是“又训练了一个更强的新模型”。它更像给 DeepSeek-V4 推理服务装上一条加速流水线：先让小草稿模块提前猜一段 token，再让大模型一次性验证；能过的留下，过不了的由大模型修正。

这类方法叫 speculative decoding。它的难点不在“猜得越多越好”，而在两个更工程的问题：草稿别前后打架，验证别浪费大模型算力。

DSpark 的判断很直接：大模型服务变慢，很多时候不是模型不会算，而是把 GPU 时间花在了低价值 token 上。
...
这个流程的收益很清楚：草稿猜得准，大模型一次 forward pass 就能推进多个 token。
...
我的理解是：先把草稿摊开，再用一支很轻的红笔，把句子内部的前后依赖补上。

![不同草稿长度下的接受长度与延迟开销。图源：DeepSeek DSpark paper Figure 4](imgs/figure-4-proposal-length-latency.png)

Figure 4 不必盯着某一个点看，重点是趋势：草稿长度变长时，DSpark 和 DFlash 的差距会拉大；但 sequential head 带来的延迟开销很小。

所以 DSpark 没有靠“多堆一个大模型”换质量。它是在草稿尾部最容易掉链子的地方，加了一点便宜的依赖建模。
...
论文要说明的是，DSpark 把 serving 系统的吞吐和交互速度边界往外推了。
...
按这张图看，同样想保证用户感觉“够快”，DSpark 能支撑更高总吞吐；同样总吞吐下，DSpark 能让单个用户拿到更快生成速度。

更严格的 SLA 下，论文还给出了 661% 和 406% 这类很大的吞吐提升数字。但论文自己也提醒，这些点更多说明 baseline 已经接近运行边界，不应当当成常规乘数收益来宣传。

这也是我会信这篇论文的原因之一：它没有只报最大数字，还把数字出现的位置说清楚。

## 负载变了，验证预算也得变

如果只保留 DSpark 的一个工程启发，我会选这一条：
...
Figure 8 下面两张图适合看调度行为：并发请求越多，DSpark 分配给每个请求的平均验证预算越会下降。MTP baseline 基本固定在 2 个 token 附近，而 DSpark 会在大约 4 到 6 个 token 之间动态调整，再随着负载上升逐步收紧。
...
很多推理加速方案在单请求 benchmark 上很漂亮，到了多用户服务就失真。不是算法完全没用，而是验证 token 会抢 batch 容量，动态长度会让底层执行变复杂。
...
建议把上面那张表收藏起来。下次看到 speculative decoding 的 speedup 数字，别只看“快了几倍”。先问三件事：接受长度怎么来的，验证浪费怎么控，负载变高时吞吐边界有没有一起变好。
