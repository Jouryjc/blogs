---
title: "别只盯着模型：Codex 难抄的是这套 Harness"
...
![](illustrations/codex-harness/01-harness-layers.png)
...
![](illustrations/codex-harness/02-app-server-architecture.png)
...
所以“有 JSON-RPC”并不自动等于稳定。协议的稳定来自四件事：少量清楚的原语、显式能力协商、与二进制匹配的类型，以及客户端愿意处理未知与失败。
...
![](illustrations/codex-harness/03-thread-turn-item.png)
...
Harness 统一的是控制流和策略接口。实际隔离仍要落到每种工具、凭证和运行环境上。
...
下次评估一个 Coding Agent，别只问它接了什么模型、榜单多少分。把上面的八项检查表拿出来，逐项写下现有证据和缺口，再看它在任务跑到第 30 分钟、等待审批、客户端断线之后，还剩多少工程确定性。
