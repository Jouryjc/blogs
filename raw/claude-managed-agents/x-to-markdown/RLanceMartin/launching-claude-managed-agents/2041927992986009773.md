...
requestedUrl: "https://x.com/RLanceMartin/status/2041927992986009773"
author: "Lance Martin (@RLanceMartin)"
authorName: "Lance Martin"
authorUsername: "RLanceMartin"
authorUrl: "https://x.com/RLanceMartin"
tweetCount: 1
coverImage: "https://pbs.twimg.com/media/HFZdAn_bwAEOkmV.jpg"
tags:
  - type/thread
...
- [Engineering blog](https://github.com/anthropics/skills/tree/main/skills/claude-api): The design of Claude Managed Agents
- [Docs](https://www.anthropic.com/engineering/managed-agents): Onboarding, quickstart, overview of the CLI and SKDs 

Why Claude Managed Agents

The Claude [messages API](https://claude.com/blog/harnessing-claudes-intelligence) is a direct gateway to the model: it accepts messages and returns content blocks. Agents built on the messages API use a harness to route Claude’s tool calls to handlers and manage context. This poses a few challenges:

- Harnesses need to keep up with Claude – I recently wrote a blog [here](https://platform.claude.com/docs/en/managed-agents/quickstart) focused on building agents using Claude API primitives to handle tool orchestration and context management. But agent harnesses encode assumptions about what Claude can’t do. These assumptions grow stale as Claude gets more capable and can [bottleneck](https://platform.claude.com/docs/en/managed-agents/onboarding) Claude’s performance. Harnesses need to be continually updated to keep pace with Claude.
...
Addressing these challenges is important because we expect future Claude to run over days, weeks, or months on humanity's greatest challenges. The [Claude Agent SDK](https://github.com/chenglou/pretext) was a first step, providing an excellent general purpose agent harness. Claude Managed Agents is the next step in this progression: a system with the harness and managed infrastructure designed to support safe, reliable execution over the time-horizon that we expect Claude to work.

How to get started

An easy way to onboard is to use our open source [claude-api skill](https://platform.claude.com/docs/en/managed-agents/quickstart), which works out of the box in Claude Code. Get the latest version of Claude Code and run the following sub-command for Claude Managed Agents onboarding. I’m excited about skills as a way to onboard to new features, and have used this skill extensively:

![](https://pbs.twimg.com/media/HFZV93JaQAAbdHf.jpg)

Also see our [docs](https://platform.claude.com/docs/en/managed-agents/quickstart) for quickstart with the SDKs or CLI, and prototype agents [in Claude Console](https://rlancemartin.github.io/2025/07/30/bitter_lesson/).

Use cases

You can see our [Claude blog](https://metr.org/time-horizons/) for a number of interesting examples. Some of the common patterns I’ve noticed across these examples and my own work:
...
![](https://pbs.twimg.com/tweet_video_thumb/HFZWVAZawAAUYUX.jpg)
...
- Long-horizon tasks: Long-running tasks are an area where I think Managed Agents will be particularly useful. I’ve explored this by forking @karpathy's auto-research repo and exploring a few different ideas. For example, I recently took @_chenglou’s excellent [pretext library](https://platform.claude.com/docs/en/api/client-sdks) and had a Managed Agent explore ways to apply it to our engineering blog content.

![](https://pbs.twimg.com/media/HFZXR5pakAA8c4K.jpg)
...
See [docs](https://claude.com/blog/claude-managed-agents) here:

- [SDKs](https://rlancemartin.github.io/2025/07/30/bitter_lesson/) – These are code-facing: import them in your app to drive sessions at runtime. Six languages have Managed Agents support: Python, TypeScript, Java, Go, Ruby, PHP.
- [CLI](https://platform.claude.com/docs/en/managed-agents/quickstart) – Terminal-facing: every API resource (agents, environments, sessions, vaults, skills, files) is exposed as a subcommand.
...
I'm excited about projects exploring different patterns of multi-agent orchestration or long-running tasks. One of the frustrations [I've written about in the past](https://platform.claude.com/docs/en/build-with-claude/working-with-messages) is keeping agent harnesses up with model capabilities. Claude Managed Agents handles the agent harness and infrastructure for you, allowing for explorations on top of the agent as a new core primitive in the Claude API.