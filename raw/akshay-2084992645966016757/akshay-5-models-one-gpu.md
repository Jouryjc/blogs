...
- The Superlinked Inference Engine. Three primitives, and the five things happening underneath.
- Proving It Against a Real Document. A flood insurance claim through docling, GLiNER2, a reranker, Grounding DINO, and Qwen3.5.
- Run It Yourself. Notebooks, setup, and placing the models across one GPU or two.
...
1. Dedicated GPUs are simpler but scale hardware one-to-one with the pipeline.
1. Shared GPUs use hardware more efficiently but need something to coordinate the models on the card.
...
So what does a serving layer need to do to make shared GPUs work?
...
![](imgs/img-21.jpg)
Now we will put that architecture against a real use case and see what happens when one pipeline actually needs several different models.
...
print(result["data"])
# [
#   {"label": "insured name", "text": "...", "score": 0.97, "start": 0, "end": 0},
#   {"label": "date and time loss", "text": "...", "score": 0.96, "start": 0, "end": 0},
#   ...
# ]
```
...
print(result["data"])
# [
#   {"label": "furniture", "score": 0.509, "bbox": [412, 890, 1180, 1640]},
#   {"label": "standing water", "score": 0.276, "bbox": [50, 1500, 2900, 2280]},
# ]
```
...
#     {
#       "severity": "blocking",
#       "detail": "Proof of Loss lacks required signature and date."
#     }
#   ]
# }
```
...
Check out Superlinked GitHub (100% Open Source) →
